...
class SMPL(object):
    def __init__(self):
        ...
        #self.joint_regressor_h36m = np.load(osp.join('../data', 'Human36M', 'J_regressor_h36m_correct.npy')).astype(np.float32)
        #self.joint_regressor_coco = np.load(osp.join('../data', 'MSCOCO', 'J_regressor_coco.npy')).astype(np.float32)
    ...
    def get_layer(self, gender):
        return SMPL_Layer(gender=gender, model_root='./smplpytorch/smplpytorch/native/models')
